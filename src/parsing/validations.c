...
/*   Updated: 2024/04/19 21:58:51 by dehamad          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool	validate_quotes(t_data *data)
{
	...
	while (data->line[i])
	{
		if (ft_isquote(data->line[i]))
		{
			if (!quotes)
				quotes = data->line[i];
			else if (quotes == data->line[i])
				quotes = 0;
		}
		i++;
	}
	if (quotes)
		return (print_error(QUOTES_ERR), false);
	return (true);
}
	...
	while (data->line[i])
	{
		if (data->line[i] == '>' || data->line[i] == '<')
		{
			if (data->line[i] == data->line[i + 1])
				i++;
			if (!scan_context(data->line, i + 1, data->line[i], '+'))
				return (false);
		}
		i++;
	}
