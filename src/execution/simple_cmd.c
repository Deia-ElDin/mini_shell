...
/*   Updated: 2024/06/03 09:07:28 by melshafi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	call_child(char *cmd, t_ast *ast, t_data *data)
{
	...
	if (ast->head->prev_exists && !ast->heredoc->heredoc_exists)
	{
		dup2(ast->head->prev_pipe[READ_END], STDIN_FILENO);
		close(ast->head->prev_pipe[READ_END]);
	}
	else if (ast->head->prev_exists && ast->heredoc->heredoc_exists)
...
static void	call_parent(pid_t pid, char *path, t_ast *ast, t_data *data)
{
	...
	if (ast->prev_exists && !ast->right->heredoc->heredoc_exists)
		close(ast->prev_pipe[READ_END]);
	else if (ast->prev_exists && ast->right->heredoc->heredoc_exists)
